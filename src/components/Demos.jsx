...
export const DEMOS = [
  ...
  { name: 'AIR Business Center', type: ['Landing Page', 'Animated', 'Premium'],          stat: '+38% new patients', color: '#0d1f1a', image: '/aircenter-hero.jpg', imgPosition: '50% 45%', url: 'https://aircenter.space/' },
  ...
  { name: 'Centenario',          type: ['Animated', 'Premium'], stat: '+51% engagement', color: '#d5d5d8', image: '/lamborghini-hero.jpeg', url: 'https://lamborghini-centenario-showcase.vercel.app/' },
]

// Curated favourites — shown in their own section above the full grid.
const FAVOURITES = ['Meridian Studio', 'Cuts & Edges', 'Halcyon Spa', 'Elixir Hotel', 'Piment', 'Centenario']
  ...
  return (
    <div style={{ minHeight: '100vh', padding: 'clamp(40px,6vw,80px) clamp(16px,4vw,60px)', fontFamily: "'Itoya','Helvetica Neue',Arial,sans-serif" }}>
      <div style={{ marginBottom: 64 }}>
        ...
        >
          These are quality examples — sites built to the standard we hold ourselves to. Not our own work, but the exact quality, performance, and results your site will deliver.
        </motion.p>
      </div>

      {/* Favourites — curated picks */}
      {FAVOURITES.length > 0 && (
        <div style={{ marginBottom: 64 }}>
          <SectionLabel title="Favourites" sub="A handful we're especially proud to point to." />
          <div style={GRID}>
            {FAVOURITES.map((demo, i) => (
      ...
      <SectionLabel title="Browse all" delay={0.05} />
      <motion.div
